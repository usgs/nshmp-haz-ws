...
function plot_curves(plot_info){
  ...
  var x_scale       = plot_info.x_scale;            // Get the starting X scale 
  var y_scale       = plot_info.y_scale;            // Get the starting Y scale
  var margin        = plot_info.margin;             // Get the margin values
  var resize_id     = plot_info.resize;             // Get the resize DOM id
  var xaxis_btn     = plot_info.xaxis_btn;          // Get the X axis button id 
  var yaxis_btn     = plot_info.yaxis_btn;          // Get the Y axis button id
  var tooltip_text  = plot_info.tooltip_text;       // Get the tooltip text
  var plot_div_id   = document.getElementById(plot_id);   // Get the plot dom
  var resize_div_id = document.getElementById(resize_id); // Get the resize dom 
  //-------------------------------------------------------------------------------


  //......................... Set Plot Panel Content Height .......................
  $(".plot-panel").removeClass("hidden");                 // Set plot visible
  var panel_id = plot_div_id.parentNode;                  // Get the parent of the plot content
  var header_height = panel_id                            // Get the plot panel header height
    .getElementsByClassName("panel-heading")[0]
    .clientHeight;
  var footer_height = panel_id                            // Get the plot panel footer height
    .getElementsByClassName("panel-footer")[0]            
    .clientHeight;
  var pad = header_height + footer_height + "px";         // Get the total height of header and footer in px
  plot_div_id.style.height = "calc(100% - "+pad+")";      // Set the plot content height 
  //-------------------------------------------------------------------------------

  
  //.......................... Set Axes Scale Buttons .............................
  var xscale_check = $("#"+xaxis_btn + " [class*=active] input").val();
  var yscale_check = $("#"+yaxis_btn + " [class*=active] input").val();
  
  if (xscale_check){                                  // If a button is already pressed, set global X scale variable
    x_scale = xscale_check;
  }else{                                              // Else set X scale from plot_info
    $("#"+xaxis_btn+" [value*="+x_scale+"]")
      .parent()
      .addClass("active"); 
  }
  
  if (yscale_check){                                  // If a button is already pressed, set global Y scale variable
    y_scale = yscale_check;
  }else{                                              // Else set Y scale from plot_info
    $("#"+yaxis_btn+" [value*="+y_scale+"]")
      .parent()
      .addClass("active"); 
  }
  //-------------------------------------------------------------------------------
