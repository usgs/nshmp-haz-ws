...
class D3LinePlot extends D3View{
  ...
  */
  plotData(whichPlot){
    ...
    if (whichPlot == "lower"){
      panel = _this.lowerPanel;
      options = panel.options;
    }else{
      panel = _this.upperPanel;
      options = panel.options; 
    }
    // Get color scheme
    ...
    //........................ Get Values ......................................
    panel.xBounds = D3LinePlot.getXScale(_this.options);
    panel.xExtremes = D3LinePlot.getXExtremes(panel.data);
    panel.xBounds.range([0, panel.plotWidth])
        .domain(panel.xExtremes)
        .nice();

    panel.yBounds = D3LinePlot.getYScale(_this.options);
    panel.yExtremes = D3LinePlot.getYExtremes(panel.data);
    ...
    let panelBodyGeom = panel.plotBodyEl.getBoundingClientRect();
    panel.scale = panel.svgWidth/panelBodyGeom.width;
    //--------------------------------------------------------------------------
    ...
    d3.select(panel.xAxisEl)
        .select(".x-tick")
        .attr("transform","translate(0,"+ panel.plotHeight+")") 
        .style("font-size", options.tickFontSize)
        .call(d3.axisBottom(panel.xBounds));
    
    // Y Tick marks
    d3.select(panel.yAxisEl)
        .select(".y-tick")
        .style("font-size", options.tickFontSize)
        .call(d3.axisLeft(panel.yBounds));
    ...
    d3.select(panel.xAxisEl)
        ...
        .attr("y", options.marginBottom-
            (options.marginBottom- panel.xAxisHeight)/2)
        .text(panel.xLabel);
    ...
    // Create legend 
    if (options.showLegend) D3LinePlot.setLegend(panel);
    ...
    d3.select(panel.allDataEl)
        ...
        .on("mouseover",function(d,i){
          let panelBodyGeom = panel.plotBodyEl.getBoundingClientRect();
          _this.scale = panel.svgWidth/panelBodyGeom.width;
    ...
    $(window).resize(function(){
      ...
      panel.scale = _this.svgWidth/panelBodyGeom.width;
  ...
  */
  static plotRedraw(_this, panel){
    ...
    panel.xBounds
        ...
        .domain(D3LinePlot.getXExtremes(panel.data))
        .nice();
    ...
    panel.yBounds
        ...
        .domain(D3LinePlot.getYExtremes(panel.data))
        .nice()
